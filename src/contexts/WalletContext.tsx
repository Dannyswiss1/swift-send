    ...
    const checkInterval = setInterval(() => {
      ...
      if (window.freighter) {
        ...
        if (indicators.some((indicator) => indicator)) {
          ...
          window.dispatchEvent(new Event("freighter-detected"));
          console.log(
            "🔍 Freighter extension indicators found, API still loading...",
          );
        }
  ...
  const availableWallets: StellarWallet[] = useMemo(() => {
    ...
    const detectFreighter = () => {
      ...
      console.log("🔍 Freighter Detection Debug:", {
        ...
        hasIsAllowed: window.freighter
          ? typeof window.freighter.isAllowed
          : "N/A",
        hasRequestAccess: window.freighter
          ? typeof window.freighter.requestAccess
          : "N/A",
        hasGetPublicKey: window.freighter
          ? typeof window.freighter.getPublicKey
          : "N/A",
    ...
    return [
      {
        ...
        connect: async () => {
          ...
          if (window.freighter) {
            ...
            console.log(
              "⏳ Freighter API not immediately available, waiting...",
            );
          }
          ...
          while (!window.freighter && attempts < maxAttempts) {
            ...
            if (attempts % 20 === 0) {
              console.log(
                `🔍 Still waiting for Freighter... (${((attempts * 100) / 1000).toFixed(1)}s/${(maxAttempts * 100) / 1000}s)`,
              );
            }
          }
          ...
          try {
            ...
            if (!isAllowed) {
              ...
              const { isAllowed: approved } =
                await window.freighter.isAllowed();
              console.log("User approved:", approved);

              if (!approved) {
                throw new Error(
                  "Please approve the connection in the Freighter popup to continue.",
                );
              }
            }
            ...
            if (
              ...
              throw new Error(
                "Connection cancelled. Please try again and approve the Freighter popup.",
              );
            }
            throw error;
          }
      ...
      },
      {
        ...
        connect: async () => {
          ...
          toast.warning("Demo Wallet Connected", {
            description:
              "⚠️ This is a demonstration wallet. No real transactions will be made.",
            duration: 5000,
          });
      ...
      },
      {
        ...
        connect: async () => {
          if (!window.rabet) {
            throw new Error(
              "Rabet wallet extension not found. Please install it and refresh this page.",
            );
          }
  ...
  const connectWallet = async (provider: WalletProvider) => {
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({ provider, publicKey: account.publicKey }),
      );
      ...
      if (error.message.includes("not installed")) {
        toast.error(`Please install ${provider} wallet to continue`, {
          ...
          action: {
            ...
            onClick: () =>
              window.open(
                "https://stellar.org/ecosystem/projects?tab=wallets",
                "_blank",
              ),
          },
        });
